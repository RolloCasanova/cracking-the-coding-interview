...
func Test_returnKthToLast(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    args{ll: &node{Value: "a", Next: &node{Value: "b", Next: nil}}, k: 0},
			want:    "",
			wantErr: true,
		},
		{
			name:    "a value of k greater than the length of the list should return an error",
			args:    args{ll: &node{Value: "a", Next: &node{Value: "b", Next: nil}}, k: 3},
			want:    "",
			wantErr: true,
		},
		{
